...
# СВЕРКА «ТЕКСТ СОДЕРЖИТ» ИМЕЕТ ТРИ ИСХОДА, А НЕ ДВА, И ТРЕТИЙ ОБЯЗАН НАЗЫВАТЬСЯ.
#
# Писалась она конвейером `"$(grep_probe "$X" 'P')"`, и
# под `set -o pipefail` (строка 13) этот конвейер отвечает `нет` не только тогда, когда
# подстроки нет: SIGPIPE писателю, отказ fork под нагрузкой, любой ненулевой код ЛЮБОГО
# звена — всё это тот же самый `нет`, то есть «правило нарушено». Диагностика при этом
# уходит в stderr, а процессный тест (`review-delivery.process.test.ts`) показывает
# читателю ТОЛЬКО stdout — причина отказа невидима по построению.
#
# ЗАМЕР (тред 185): прогон 34377052327 (голова caf3fd0cd) покраснел РОВНО одной такой
# сверкой — `назван ЧУЖОЙ парк с номером`, — а следующий прогон 34378904884 на голове
# 47ec05764 зелен, при том что диффа в `.github/scripts/**` между этими головами нет
# вовсе. Один и тот же код на одном и том же входе ответил по-разному, и что именно не
# состоялось, из прогона узнать нечем: ответ `нет` съел вопрос.
#
# Отсюда `grep_probe`. Писателя-процесса нет вовсе (here-string вместо конвейера) —
# схлопывать SIGPIPE неоткуда; сам grep читается КОДОМ: 0 → `да`, 1 → `нет`, любой иной →
# третий ответ, который не совпадёт ни с одним ожиданием и назовёт себя по имени. Семантика
# самого grep не трогается — флаги и шаблон едут ему как есть, поэтому ни одна сверка не
# меняет смысла.
#
