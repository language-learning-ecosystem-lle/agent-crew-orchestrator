...
check "исход 1 — факт повторов виден в выводе" "да" \
  "$(grep -qF 'прочитано с попытки 3 из 3' "$ERR_FILE" && echo да || echo нет)"
check "исход 1 — отказавшие попытки названы поимённо" "да" \
  "$(grep -qF 'не прочитано (попытка 1 из 3' "$ERR_FILE" && echo да || echo нет)"
...
check "исход 2 — причина названа отказом ЧТЕНИЯ, а не отсутствием поля" "да" \
  "$(grep -qF 'НЕ ПРОЧИТАНО за 3 попытки' "$ERR_FILE" && echo да || echo нет)"
check "исход 2 — сказано, что это НЕ значит «поля нет»" "да" \
  "$(grep -qF 'Это НЕ значит, что поля нет' "$ERR_FILE" && echo да || echo нет)"
...
check "контроль мутации: шаблон неохраняемого чтения ловит СТАРУЮ форму" "да" \
  "$(printf '          PR_BODY=$(gh pr view "$PR" --json body -q .body)\n' \
     | grep -E '^[[:space:]]*[A-Z_]+=\$\(gh pr view ' | grep -qv '||' && echo да || echo нет)"
check "контроль мутации: и НЕ ловит новую" "нет" \
  "$(printf '          PR_BODY=$(review_pr_field "$PR" body) || PR_BODY=""\n' \
     | grep -E '^[[:space:]]*[A-Z_]+=\$\(gh pr view ' | grep -qv '||' && echo да || echo нет)"
check "контроль мутации: и НЕ ловит уже охраняемую строку шага checks_on_head" "нет" \
  "$(printf '          HEAD_SHA=$(gh pr view "$PR" --json headRefOid -q .headRefOid 2>/dev/null) || HEAD_SHA=""\n' \
     | grep -E '^[[:space:]]*[A-Z_]+=\$\(gh pr view ' | grep -qv '||' && echo да || echo нет)"
check "контроль: греп читает тот самый воркфлоу" "да" \
  "$(grep -qF 'name: Итог доставок — что доехало и что нет' "$WF" && echo да || echo нет)"
