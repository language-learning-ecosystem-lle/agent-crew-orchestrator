...
# конфиг читается из `--repo . --ref origin/main` рабочего дерева роли, а единственный
...
# (`pnpm install` до всех шагов) и remote-tracking ref `origin/main` — его чекаут Actions
# сам по себе не заводит, поэтому шаг притягивает его явно. ПОЧЕМУ ИМЕННО `origin/main`, а
# не `HEAD` этого PR: `origin/main` читает не сюита, а ПРОДОВЫЙ код доставки
# (`review-delivery.sh:321,466,516`), который зовут состояния (8)/(9)/(14)/(15); менять его
# ради CI — не предмет, а читать двумя разными ref в одном прогоне значит проверять
# полусобранное. Вопрос «держится ли конфиг ТОГО, что будет влито» здесь и не задаётся: его
# задаёт шаг `config check --ref HEAD` той же джобы, раньше по порядку.
set -uo pipefail
