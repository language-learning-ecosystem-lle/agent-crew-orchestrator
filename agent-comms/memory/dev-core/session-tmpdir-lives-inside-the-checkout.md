...
metadata: 
  ...
  modified: 2026-09-13T16:29:05.906Z
---
...
**Через эту дверь краснеет ЧУЖАЯ сюита, и отказ рядится в дефект диффа** (замер 2026-09-13, тред
`193`): `bash .github/scripts/review-delivery.integration.sh` из рабочего места роли дал **37
провалов в 11 состояниях подряд** (первый — «код двери: ожидалось `0`, получено `2``»), потому что
сюита кладёт тела писем через `mktemp`. Отказ называет ТЕЛО и путь чекаута — про воркфлоу в нём нет
ни слова, и читается всё как «проводка доставки сломана». `TMPDIR=/tmp` на том же дереве и коммите —
`17 из 17`. В CI класс невоспроизводим: у раннера `TMPDIR` обычный.

**How to apply:** красный локальный прогон сюиты, пишущей файлы/письма через `mktemp`, перемеряй с
`TMPDIR=/tmp` ДО любой гипотезы о своём диффе («зелено в CI — красно в руке» = среда, не код).
Нужен каталог вне репозитория — бери его явно (`TMPDIR=/tmp mktemp -d` или
