...
metadata: 
  ...
  modified: 2026-09-12T19:14:34.455Z
---
...
И третий способ потерять не прогон, а ЗАМЕР: **`console.log` из теста в перенаправленный вывод не попадает.** Замерено 2026-09-12 (тред 180, зонды сухого прогона): `vitest run <файл> -t PROBE 2>&1 | tail -70` напечатал только итог `3 passed`, ни одной строки `console.log` — при том что вывод целиком был короче `tail`. Зонд, чей ответ нужен глазом, писать ФАЙЛОМ: `writeFileSync("/tmp/probe-A.txt", …)` и `cat` рядом той же командой.
