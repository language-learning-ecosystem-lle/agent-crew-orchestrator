...
metadata: 
  ...
  modified: 2026-09-08T17:12:16.045Z
---
...
**ГОЛЫЙ НОМЕР в слаг не разрешается ничем.** Измерено 2026-09-07 (тред 063, закрытие; предмет — «жив ли тред 116, названный в `docs/state-model.md`»): `thread show --thread 116` и `--thread 116-` отвечают `not found` и кандидатов НЕ печатают; листинга тредов у CLI нет (`tasks list` говорит только о задачах — «no tasks declared yet»), а `mail --role <r>` показывает лишь треды, ЖДУЩИЕ эту роль, — по всем пяти ролям ни одного `116-*`. То есть из связанного прогона, которому корень почты руками читать нельзя (R3: две команды — весь интерфейс), вопрос «открыт ли тред NNN» НЕ измеряется вовсе. **How to apply:** не жечь такты на подбор слага — назвать в письме, что именно НЕ измерено, а сам предмет искать там, где он записан durable: в доке на `main` ([[reading-a-ref-must-not-write-the-tree]]), а не в ленте.

**ПОПРАВКА 2026-09-08: дверь записи `NNN.M` БОЛЬШЕ НЕ ПРИНИМАЕТ, и половина «принимает и пушит» выше — историческая.** Замерено чтением кода (тред 177, факт принесён dev-core, сверен своей рукой): `thread/id.ts:29` — `THREAD_ID = /^\d{3}-/`, `unreadableThreadId` (`id.ts:41`) спрашивается в `cli.ts:2085` ДО записи и отказывает по имени («a new form of address is a change of the norm, not of this id»). То есть класс «принято на записи, невидимо на чтении» закрыт дверью (тред 086), а `047.1-devops-enablement-acceptance` — сирота из эпохи, когда дверь принимала. **How to apply:** не планировать форму `NNN.M` ни в каком механизме (раскладка рабочих мест, адреса) как «дверь такое берёт» — не берёт; всё остальное в этой заметке (невидимость сироты для счёта, отсутствие двери на закрытие) в силе.

**Из указателя (перенесено 2026-09-06, оглавление шло за потолок):** `NNN.M` дверь записи принимала и пушила, а обходчик (`^\\d{3}-`) не видел: после `new-thread --write` контрольное `mail --role <адресат>` остаётся нормой

**Из указателя (перенесено 2026-09-08, оглавление шло за потолок):** голый номер в слаг не разрешается ничем
