...
metadata: 
  ...
  modified: 2026-09-17T12:31:59.444Z
---
...
**ЭХО ПОДДЕЛЫВАЕТ И НУЛЕВОЙ СЧЁТ ИСХОДА (замерено 2026-09-17, тред `218`):** приёмка уборки требовала
«ни одного отказа», и `grep -c 'tidy-up: REFUSED'` дал **1** — попаданием оказался кусок ИСХОДНИКА
(`describeTidyUpOutcome`), отражённый в лог сессией dev-core, читавшей этот код. Якорный счёт
`grep -c '^agent-protocol: daemon — tidy-up: REFUSED'` дал 0. Класс исхода, которого «не должно быть»,
эхо превращает в «был» ровно тогда, когда кто-то рядом читал код этого исхода.
