...
**Из указателя (перенесено 2026-09-06, оглавление шло за потолок):** джоба стоит на `if: pull_request.merged == true`, поэтому пропущенные письма о merge не переигрываются, а полевую приёмку уведомителя нельзя устроить по требованию

**Из указателя (перенесено 2026-09-06, оглавление шло за потолок):** джоба стоит на `if: pull_request.merged == true` — пропущенные письма о merge не переигрываются и по требованию не мерятся
