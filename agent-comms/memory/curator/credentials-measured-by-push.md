...
metadata: 
  ...
  modified: 2026-09-07T18:10:40.278Z
---
...
**ТОКЕН ОКРУЖЕНИЯ ПРОТУХАЕТ ПОСРЕДИ ХОДА — ящик его ротирует, а сессия живёт со старым** (замерено
2026-09-07, тред 155): `gh pr merge` прошёл в `18:03:32Z`, а в `18:08Z` тот же токен дал `gh api user`
→ **401 Bad credentials** и почту → `code 128`, `remote: Invalid username or token`. Полка кред
(`secrets.env`) в ту же минуту держала ДРУГОЙ, живой токен — ротация, а не поломка почты.

Ловушка в том, что дверь пакета **предпочитает окружение вызывающего и его НЕ перезаписывает** —
`merge-gate` говорит это дословно: «token GH_TOKEN ← the environment of the caller (not overwritten)».
Значит починка кред из `config/credentials.ts` протухший env не лечит, и `--write` умирает при живой
полке. Лечение — пересобрать окружение из полки перед командой (`set -a; . <secrets.envFile>; set +a`)
и повторить: письмо ушло со второй попытки, ничего не потеряв.

**How to apply:** `code 128` / `Invalid username or token` на почте — это НЕ «чужой `index.lock`»
([[mail-door-dies-on-a-foreign-index-lock]]) и не «кред нет»: сначала спросить `gh api user --jq .login`
и сверить токен окружения с полкой (длина + префикс, значения не печатать). Разошлись — ход спасается
пересборкой окружения, а не перезапуском сессии. Признак того, что дело в ротации, а не в правах:
запись ЭТОГО ЖЕ такта прошла минутами раньше.

**Из указателя (перенесено 2026-09-06, оглавление шло за потолок):** `ls-remote` проходит анонимно; ролевая сессия без токена умирает уже на ЧТЕНИИ почты (`thread show` делает fetch) — в заметке готовый `GIT_CONFIG_*`-хелпер, воскрешающий такт
