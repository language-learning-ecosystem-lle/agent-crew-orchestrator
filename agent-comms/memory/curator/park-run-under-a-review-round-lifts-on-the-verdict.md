...
metadata: 
  ...
  modified: 2026-09-15T10:42:26.292Z
---
...
**НО «такты до вердикта не поднимают никого» верно только пока парку МЕНЬШЕ 30 минут** (замер 15.09,
тред 198, #448). Потолок парка — 30 мин, и возраст считается от ПЕРВОГО объявления: честные переносы
`declaredAt` НЕ обновляют ([[parking-mechanics]], правило носителя). Парк `run:448`, объявленный
`09:15:11Z` и дважды перенесённый (`10:02:13Z`, `10:32:46Z`), к `10:33Z` стоял 79 мин против потолка
30 — `daemon.log`: «HAS GONE STALE … the role is raised to check the outcome of that run itself». Круг
ревью длится ~6 мин, значит **парк, переживший пару итераций PR, гарантированно даёт подъём посреди
живого круга** — это не поломка лифта, и заготавливать под него класс дефекта не надо. Обнуляет
возраст только гашение+объявление в одном вдохе (`--park-lifted run:N` + `--parked-on run:N`).
...
**Исполнено в поле 13.09 (#398), и подъём даёт НЕ вердикт:** парк купил три пропущенных такта и ноль
пустых подъёмов (каталог `.orchestrator/sessions/`: `14-45-59Z`, следом сразу `14-54-50Z`), но пара
поднялась через **311 с** после вердикт-объекта — свободное место коробки заняла другая пара за 12 с
до конца круга, и такты пропускали её строкой «the ceiling of this BOX is full». **Лифт делает пару
КАНДИДАТОМ; запуск отдаёт потолок коробки** — «вердикт → мой такт» по одному лифту не планируется.
