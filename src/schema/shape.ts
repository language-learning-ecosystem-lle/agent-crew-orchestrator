...
export const CONFIG_SHAPES: Readonly<Record<number, readonly string[]>> = {
  ...
  /**
   * 26 — thread `063-state-model-rewrite`: WHICH LABEL AND WHICH WORKFLOW ARE THE ROUND OF
   * REVIEW is a declaration of the served project (`review`), not a word compiled into the
   * package. THREE new key paths and no new value: both fields are free strings — one names a
   * GitHub label, the other a workflow of the served repository — so the value half of the
   * guard does not move.
   */
  26: [
    "announcements",
  ...
  /**
   * 27 — thread `177-workspace-per-pair`: HOW MANY PAIRS «role × thread» RUN AT ONCE is a
   * declaration of the served project (`parallelism`), not the planner's compiled-in «one
   * session per role». THREE new key paths and no new value: both halves are plain positive
   * integers, so the value half of the guard does not move. The relation BETWEEN them (a box
   * ceiling below the role ceiling is refused by name) is a rule and not a pinned value —
   * this table records the vocabulary a released build accepted, and a refusal on a pair of
   * numbers has no vocabulary to record.
   */
  27: [
    "announcements",
    "announcements.force-stop",
...
export const CONFIG_VALUES: Readonly<Record<number, readonly string[]>> = {
  ...
  // 26 — thread `063-state-model-rewrite`: `review.label` and `review.workflow` pin NOTHING.
  // Both are free strings — a label of one repository and a workflow name of one repository —
  // so the value half stands still while the key half moves, and the entry is 25 verbatim.
  // Recorded rather than skipped: a version with no row here reads as a version nobody checked.
  26: [
    'roles[].capabilities[].name = "disk-free"',
    'roles[].capabilities[].name = "log-tail"',
    'roles[].capabilities[].name = "repo-refresh"',
  ...
  // 27 — thread `177-workspace-per-pair`: `parallelism.pairsPerRole` and
  // `parallelism.pairsPerInstance` pin NOTHING. Both are positive integers — how many pairs one
  // role and one box may run at once — so the value half stands still while the key half moves,
  // and the entry is 26 verbatim. Recorded rather than skipped: a version with no row here reads
  // as a version nobody checked.
  27: [
    'roles[].capabilities[].name = "disk-free"',
    'roles[].capabilities[].name = "log-tail"',
    'roles[].capabilities[].name = "repo-refresh"',
