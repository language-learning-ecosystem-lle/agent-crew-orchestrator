 ...
 */
const monitor = async (answer: number | "hang" = 200): Promise<Monitor> => {
  const paths: string[] = [];
...
type Contour = { readonly repo: string; readonly local: string };
 ...
 */
const tick = (place: Contour): Promise<{ code: number; out: string }> =>
  new Promise((resolve) => {
    const child = spawn(
      TSX,
      [
      ...
      { cwd: place.repo, stdio: "pipe", env: sandbox(configHome(place.repo)) },
    );
    let out = "";
