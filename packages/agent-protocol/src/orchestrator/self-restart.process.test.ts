 ...
 * while node loaded the modules from the developer's own checkout. `--ref` is resolved in
 * the checkout the CODE came from (that is the whole question 023.2 asks — "is what this
 * process is running behind the ref it judges by"), so on any developer branch the drift
 * is real, the box is idle and clean, and the memory of attempts lives in the temporary
 * repository, which is to say it is always empty. Without this condition the first
 * self-restart of this package's history would have fired inside its own test suite:
 * `restart --pull` against the ROLE'S OWN WORKTREE — the tree R17 resets, locks and
 * removes under the circuit, the last tree on this box anything may pull unattended.
 *
...
describe("the self-restart of a daemon serving somebody else's checkout", () => {
  it(
    ...
    () => {
      ...
      // The drift is REAL here — this is the state 023.2 measures, and the reason this
      // whole file exists: the code comes from a checkout whose HEAD is not `origin/main`.
      expect(said).toContain("the LOADED CODE is not the ref");
      // Half one: it is not silent about standing (variant (1) is the floor).
