...
export const USAGE = `usage (--ref is required everywhere except 'schema migrate', 'doctor', 'init', 'config set' and the operator's five below; --repo defaults to the repository of the current directory):
  ...
  agent-protocol new-thread   --root <mail> --ref <ref> --id <NNN-slug> --title <t> --participants <r,r> --from <role> --expects <e> [--waiting-on <role>] [--parked-on <who>] [--delivers <person>] [--park-mover <participant>] [--park-ground <fact>] [--verdict <approve|needs-fixes> --pr <n>] [--turn <explicit>] --worker <w> [--session <id>] [--raised <ts>] --body-file <p> [--write] [--no-push]
                              ...
                              # --raised <UTC stamp>: WHEN the writing session was started (081) — optional, and
                              # its absence is legal by the norm: a hand and a workflow have no such moment.
                              # A message carrying it does NOT lift a park on a person announced AFTER it —
                              # the session was raised before the park and never read it
